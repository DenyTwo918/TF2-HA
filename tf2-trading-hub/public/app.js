...
async function refresh(){
  ...
  scheduleLiveDashboardRefresh(250);
  setLog({ok:true,version:(versionAudit&&versionAudit.expected)||'5.13.57',hydrated_dashboard_load:true,message:'Dashboard loaded. Live status will keep hydrating in the background.'});
}
...
function renderPublishWizard(data){
  ...
  if(data.version&&data.ok&&data.steps===undefined&&data.candidate_draft_id===undefined&&data.classifieds_maintainer===undefined){el.innerHTML=`<p class="badText"><b>Production dashboard data mapping mismatch.</b> Refresh response was not /api/publish-wizard/status. Update to 5.13.57 or reload with Ctrl+F5.</p><pre>${esc2(JSON.stringify(data,null,2).slice(0,1000))}</pre>`;return;}
  const maint=data.classifieds_maintainer||{};
  ...
  const lastOpResult=op.lastOperationResult||{};
  const opJustReleased=!opBusy&&!!(lastOpResult.timed_out&&lastOpResult.stale_released);
  ...
  if(opJustReleased){html+=`<div class="miniRow badBox"><b>Expired lock released</b><small>Operation ${esc2(lastOpResult.type||'operation')} exceeded its timeout and was released automatically by the watchdog. elapsed ${esc2(fmtElapsed(Math.round((lastOpResult.elapsed_ms||0)/1000)))} · reason ${esc2(lastOpResult.reason||'operation_watchdog')}</small></div>`;}
  ...
  html+=`<div class="productionMetric ${maint.enabled?'okBox':'warnBox'}"><span>Maintainer</span><strong>${maint.enabled?'ON':'OFF'}</strong><small>${maint.enabled?('fill '+esc2(maint.active_total_listings||0)+'/'+esc2(maint.backpack_tf_account_listing_cap||maint.max_total_active_listings||600)+' · free '+esc2(maint.free_listing_slots||0)):'Maintainer is OFF. Enable to allow Maintain now to run.'}</small><label class="dashSwitch"><input type="checkbox" id="prodMaintainerToggle" ${maint.enabled?'checked':''} ${!publishModeOk?'disabled':''}><span>${maint.enabled?'Maintainer ON':'Maintainer OFF'}</span></label></div>`;
