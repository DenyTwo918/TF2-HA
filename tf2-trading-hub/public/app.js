...
async function refreshLiveDashboard(reason='poll'){
  ...
  try{
    ...
    const bot = (wizard&&wizard.bot)||(lite&&lite.bot)||{};
    const botText = bot.enabled ? 'Bot ON' : 'Bot OFF';
    renderLiveDashboardStatus('ok', changed?'Live dashboard updated from new event':'Live dashboard watching', `reason ${reason} · ${botText} · lite${wizard?'+full':''}`);
    if(changed && wizard){
      Promise.allSettled([
...
async function refresh(){
  ...
  scheduleLiveDashboardRefresh(250);
  setLog({ok:true,version:(versionAudit&&versionAudit.expected)||'5.13.53',hydrated_dashboard_load:true,message:'Minimal Bot dashboard loaded. Live status will keep hydrating in the background.'});
}
...
function renderPublishWizard(data){
  ...
  const bot=data.bot||{enabled:!!data.bot_enabled,mode:data.bot_enabled?'on':'off'};
  ...
  const opAge=Math.max(0,Math.round(opAgeMs/1000));
  ...
  const backpack=data.backpack_summary||{};
  const inv=data.inventory_summary||{};
  const lastOp=op.lastOperationResult||{};
  const lastErr=(lastOp&&lastOp.ok===false)?(lastOp.error||lastOp.reason||'operation failed'):(data.status_warnings&&data.status_warnings[0])||'';
  const botOn=!!bot.enabled;
  let html=`<div class="productionHero ${botOn?'okBox':'softBox'}">`;
  html+=`<div><p class="eyebrow">Bot</p><h3>${botOn?'Bot is ON':'Bot is OFF'}</h3><p class="muted">${botOn?'Guarded automatic mode is enabled. Heavy actions still use the single-flight lock.':'Automatic maintainer/publishing is disabled. This is the safest default.'}</p></div>`;
  html+=`<label class="dashSwitch bigBotSwitch"><input type="checkbox" id="prodBotToggle" ${botOn?'checked':''} ${opBusy?'disabled title="Operation already running"':''}><span>${botOn?'ON':'OFF'}</span></label></div>`;
  if(opBusy){html+=`<div class="miniRow warnBox"><b>Running: ${esc2(op.activeOperationType||op.type||'operation')}</b><small>started ${esc2(op.activeOperationStartedAt||op.started_at||'')} · elapsed ${esc2(fmtElapsed(opAge))} · stage ${esc2(op.last_stage||op.lastStage||'running')} · buttons are locked to prevent overlapping Backpack.tf syncs.</small></div>`;}
  html+=`<div class="productionGrid minimalBotGrid">`;
  html+=`<div class="productionMetric ${account.needs_setup?'warnBox':'okBox'}"><span>Account</span><strong>${account.needs_setup?'needs setup':'ready'}</strong><small>${esc2(account.steam_id64_short||account.steam_id64||'Main account')} · Steam API ${account.steam_web_api_key_saved||account.steam_api_key_saved?'saved':'missing'} · Backpack ${account.backpack_tf_access_token_saved||account.backpack_tf_token_saved?'saved':'missing'}</small></div>`;
  html+=`<div class="productionMetric ${backpack.ok?'okBox':'warnBox'}"><span>Backpack.tf</span><strong>${backpack.ok?'ready':'check'}</strong><small>listings ${esc2(backpack.listings||0)} · prices ${esc2(backpack.prices||0)} · mode ${esc2(backpack.write_mode||data.backpack_tf_write_mode||'guarded')} · live writes ${backpack.live_writes_enabled?'ON':'OFF'}</small></div>`;
  html+=`<div class="productionMetric ${Number(inv.items||0)>0?'okBox':'warnBox'}"><span>Inventory</span><strong>${esc2(inv.items||0)} items</strong><small>priced ${esc2(inv.priced||0)} · unpriced ${esc2(inv.unpriced||0)} · value ${esc2(inv.estimated_value_ref||0)} ref</small></div>`;
  html+=`<div class="productionMetric ${lastOp&&lastOp.ok===false?'warnBox':'okBox'}"><span>Last operation</span><strong>${esc2(lastOp.type||'none')}</strong><small>${lastOp.completed_at?esc2(lastOp.completed_at):'no completed operation yet'} ${lastOp.elapsed_ms?`· ${esc2(fmtElapsed(Math.round(lastOp.elapsed_ms/1000)))}`:''}</small></div>`;
  html+=`<div class="productionMetric ${lastErr?'warnBox':'okBox'}"><span>Last error</span><strong>${lastErr?'warning':'none'}</strong><small>${esc2(lastErr||'No recent error reported.')}</small></div>`;
  html+=`</div>`;
  html+=`<details class="softDetails advancedSection"><summary>Advanced manual tools</summary><div class="buttonRow productionActions"><button id="prodRunLocalWorkflow" ${opBusy?'disabled title="Operation already running"':''}>Run workflow</button><button id="refreshPublishWizard">Refresh</button></div><p class="muted">Maintainer, startup rebuild, archive and auto-sell controls are hidden from the main UI to reduce crashes and accidental live actions.</p></details>`;
  el.innerHTML=html;
}
...
document.addEventListener('click',async e=>{
  ...
  if(pub){
    ...
    return;}
  const botToggle=e.target.closest('#prodBotToggle,#botToggleTop');
  if(botToggle){try{const enabled=!!botToggle.checked;setPublishWizardStatus(enabled?'Turning bot ON…':'Turning bot OFF…');const d=await api('/api/bot/toggle',{method:'POST',body:JSON.stringify({enabled})});setLog(d);const wizard=await api('/api/publish-wizard/status');renderPublishWizard(wizard);setPublishWizardStatus(enabled?'Bot is ON. Guarded automatic mode may run.':'Bot is OFF. Automatic maintainer/publishing is disabled.');scheduleLiveDashboardRefresh(500);}catch(err){setPublishWizardStatus('Bot toggle failed.',err.body||err.message);setLog(err.body||err.message);}return;}
