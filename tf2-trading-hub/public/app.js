...
async function refresh(){
  ...
  scheduleLiveDashboardRefresh(250);
  setLog({ok:true,version:(versionAudit&&versionAudit.expected)||'5.13.55',hydrated_dashboard_load:true,message:'Dashboard loaded. Live status will keep hydrating in the background.'});
}
...
function renderPublishWizard(data){
  ...
  if(data.version&&data.ok&&data.steps===undefined&&data.candidate_draft_id===undefined&&data.classifieds_maintainer===undefined){el.innerHTML=`<p class="badText"><b>Production dashboard data mapping mismatch.</b> Refresh response was not /api/publish-wizard/status. Update to 5.13.55 or reload with Ctrl+F5.</p><pre>${esc2(JSON.stringify(data,null,2).slice(0,1000))}</pre>`;return;}
  const maint=data.classifieds_maintainer||{};
