...
// 5.13.37: bounded log render.  Earlier builds dumped the full JSON of every
...
function renderCredentials(data){
  ...
  // 5.13.37: the backend returns account_status; older UI code looked only at data.accounts,
  // so saved credentials were displayed as missing even though the vault was written correctly.
  ...
  if(select){
    select.innerHTML='<option value="main">Main account</option>';
    select.value='main';
  }
  const steamSaved=Boolean(main.steam_web_api_key_saved||main.steam_api_key_saved||main.steam_api);
  const backpackSaved=Boolean(main.backpack_tf_token_saved||main.backpack_tf_access_token_saved||main.backpack_tf||main.backpack_tf_api_key_saved);
  ...
  const readiness=(main.readiness||data.readiness||'').toString();
  const ready=Boolean(steamIdSaved&&steamSaved&&backpackSaved);
  el.innerHTML=`<div class="mainAccountSummary ${ready?'ready':'needs'}">
    <div class="accountCardTop"><b>${esc(main.label||'Main account')}</b>${pill(ready?'ready':'needs setup',ready?'ok':'warn')}</div>
    <div class="metrics compactMetrics accountSimpleMetrics">
      ...
      ${metric('Backpack token',backpackSaved?'saved':'missing')}
      ${metric('Live scope','Main only')}
    </div>
    <p class="muted">To switch account: paste the new SteamID64, Steam Web API key and Backpack.tf token above, then click <b>Save main account</b>. This card reads only the canonical vault status.</p>
    ${readiness?`<small class="muted">Readiness: ${esc(readiness)}</small>`:''}
  ...
  if(adv){
    const rows=accounts.map(a=>`<div class="accountCard"><div class="accountCardTop"><b>${esc(a.label||a.id||'Account')}</b>${pill(a.role_label||a.role||'Account','ok')}</div><p><b>SteamID64:</b> ${esc(a.steam_id64_short||a.steam_id64||(a.steam_id64_saved?'saved':'missing'))}</p><p><b>Credentials:</b> Steam ${a.steam_web_api_key_saved||a.steam_api_key_saved||a.steam_api?'saved':'missing'} · Backpack ${a.backpack_tf_token_saved||a.backpack_tf_access_token_saved||a.backpack_tf?'saved':'missing'}</p><div class="buttonRow"><button class="useCredentials" data-id="${esc(a.id||'main')}">Use credentials target</button>${String(a.id||'main')==='main'?'':`<button class="removeAccount dangerSoft" data-id="${esc(a.id)}">Remove</button>`}</div></div>`).join('');
...
// ── 5.13.37 – Live Dashboard (lite poll + adaptive backoff) ───────────
...
async function refresh(){
  // 5.13.37: hydrated fast dashboard.  The first paint must not show confusing
  ...
  scheduleLiveDashboardRefresh(250);
  setLog({ok:true,version:(versionAudit&&versionAudit.expected)||'5.13.37',hydrated_dashboard_load:true,message:'Dashboard loaded. Live status will keep hydrating in the background.'});
}
...
async function saveSelectedCredentials(extra={}){
  // 5.13.37: verified save.  The UI only clears secret fields after the backend
  ...
  try{
    ...
    const saveVerified=!!(finalData.save_verified||data.save_verified||((main.steam_id64_saved||main.steam_id64)&&(main.steam_web_api_key_saved||main.steam_api_key_saved)&&main.backpack_tf_token_saved));
    if(saveVerified){
...
function renderPublishWizard(data){
  ...
  if(data.version&&data.ok&&data.steps===undefined&&data.candidate_draft_id===undefined&&data.classifieds_maintainer===undefined){el.innerHTML=`<p class="badText"><b>Production dashboard data mapping mismatch.</b> Refresh response was not /api/publish-wizard/status. Update to 5.13.37 or reload with Ctrl+F5.</p><pre>${esc2(JSON.stringify(data,null,2).slice(0,1000))}</pre>`;return;}
  const maint=data.classifieds_maintainer||{};
