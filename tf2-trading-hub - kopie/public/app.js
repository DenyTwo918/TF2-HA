...
async function refresh(){
  ...
  scheduleLiveDashboardRefresh(250);
  setLog({ok:true,version:(versionAudit&&versionAudit.expected)||'5.13.52',hydrated_dashboard_load:true,message:'Dashboard loaded. Live status will keep hydrating in the background.'});
}
...
function renderPublishWizard(data){
  ...
  if(data.version&&data.ok&&data.steps===undefined&&data.candidate_draft_id===undefined&&data.classifieds_maintainer===undefined){el.innerHTML=`<p class="badText"><b>Production dashboard data mapping mismatch.</b> Refresh response was not /api/publish-wizard/status. Update to 5.13.52 or reload with Ctrl+F5.</p><pre>${esc2(JSON.stringify(data,null,2).slice(0,1000))}</pre>`;return;}
  const maint=data.classifieds_maintainer||{};
  ...
  const maintAutoRun=!!(maint.auto_run_enabled);
  html+=`<div class="productionMetric ${!maintAutoRun?'warnBox':''}"><span>Maintainer</span><strong>${maint.enabled?(maintAutoRun?'Auto: ON':'Manual only'):'Paused'}</strong><small>${!maintAutoRun?'Automatic maintainer disabled for safety. Use Maintain now manually. Set persistent_classifieds_maintainer_auto_run_enabled=true to enable auto cycles.':('next '+esc2(nextRun)+' · interval '+esc2(maint.interval_minutes||'?')+' min')} · max ${esc2(maint.max_publishes_per_cycle||20)}/cycle · fill ${esc2(maint.active_total_listings||0)}/${esc2(maint.backpack_tf_account_listing_cap||maint.max_total_active_listings||600)} · free ${esc2(maint.free_listing_slots||0)} · target buy ${esc2(maint.target_active_buy_listings||600)}</small><label class="dashSwitch"><input type="checkbox" id="prodMaintainerToggle" ${maint.enabled?'checked':''} ${!publishModeOk?'disabled':''}><span>${maint.enabled?(maintAutoRun?'Auto maintain on':'Manual only – auto disabled'):'Auto maintain paused'}</span></label></div>`;
