...
async function computeLiquidItems() {
  ...
  const maxBuy = Math.min(200, Math.max(1, Number(opts.max_auto_buy) || 150));
...
// Parse "Item Name=1.11, Other Item=9.5" → { 'Item Name': 1.11, 'Other Item': 9.5 }
function parsePriceOverrides(str) {
  const out = {};
  if (!str) return out;
  for (const part of str.split(',')) {
    ...
    const price = parseFloat(part.slice(eq + 1).trim());
    if (name && !isNaN(price) && price >= 0) out[name] = price;
  }
  return out;
}

async function getRefPrice(name) {
  switch (name) {
  ...
  const usd = await getSteamMarketPrice(name);
  if (usd === null || !keyPriceUsd) return null;
...
async function fetchSellListings(name, token, count = 5) {
  try {
    const data = await httpsGet(
      ...
      + '&intent=sell&tradable=1&craftable=1&quality=6&page_size=' + count
    );
...
async function getCheapestSellRef(name, token) {
  ...
  try {
    const data = await httpsGet(
      ...
      + encodeURIComponent(token)
      + '&item=' + encodeURIComponent(name)
      + '&intent=sell&tradable=1&craftable=1&quality=6&page_size=6'
    );
    const listings = (data.sell?.listings || []).filter(
      l => !appState.steam_id || String(l.steamid) !== String(appState.steam_id)
    );
...
async function syncInventoryListings() {
  ...
  try {
    ...
    for (const item of inventory) {
      ...
      let market = await getRefPrice(item.name);
      if (!market && apiToken) {
        await sleep(300);
